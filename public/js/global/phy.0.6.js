(function() {
            ...
            switch (typeof reference) {
                case 'string':
                    if (reference.match(/\#[a-zA-Z0-9-_]$/)) {
                        return d.getElementById(reference);
                    }
                    else {
                        return d.getElementsByTagName(reference);
                    }
                    break;

                case 'function':
                    window.onload = reference;
                    break;
            }
        },
        _$ = w.$;
    ...
    _phy.ajax = function(_) {
        ...
        if (_phy.array.contains(v.method.toLowerCase(), phy.ajax._methods)) {
            m = v.method;
        }
        ...
        a.onreadystatechange = function() {
            if (a.readyState == 4) {
                ...
                if (h.match('application/json') || h.match('text/xml')) {
                    r = _phy.json.parse(a.responseText);
                }
                else if (h.match('application/xml') || h.match('text/xml')) {
                    r = a.responseXML;
                }
                else {
                    r = a.responseText;
    ...
    _phy.array.extend = function(array_1, array_2) {
        if (typeof array_2 !== 'array') {
            return array_1;
        }
        else if (typeof array_1 !== 'array') {
            return array_2;
        }
    ...
    _phy.object.extend = function(object_1, object_2) {
        ...
        else if (typeof object_1 !== 'object') {
            return object_2;
        }
    ...
    (function() {
        ...
        _phy.resource.push = function(r) {
            if (!_r[r]) {
                ...
                var id = 'phy_res' + (Math.floor(Math.random() * 1000001));
                e.id = id;
                _r[r] = id;
    ...
    _phy.qs.prototype.convertToString = function(v) {
        var p = [];
    ...
    _phy.qs.prototype.convertToObject = function(p) {
        var r = {};
        var k = [];
